...
from cheems import LOGGER, download_dict, download_dict_lock, app
...
class TelegramDownloadhalper(Downloadhalper):
    ...
    def __onDownloadProgress(self, current, total):
        if self.__is_cancelled:
            ...
            self.__user_cheems.stop_transmission()
            return
        with self.__resource_lock:
    ...
    def __download(self, message, path):
        download = self.__user_cheems.download_media(message,
    ...
    def add_download(self, message, path, filename):
        _message = self.__user_cheems.get_messages(message.chat.id, message.message_id)
        media = None
