...
- Original repo is https://github.com/lzzy12/python-aria-mirror-cheems
...
- This (or any custom) repo is not supported in official cheems support group.
- So if you have any issue then check first that issue is in official repo or not, You are only allowed to report that issue in cheems support group if that issue is also present in official repo.
...
Note :- it is not a cheems Support group. It's only for discussing rubbish things bcoz i want your help to learn coding 😜🤪.


## Credits :-
- First of all, full credit goes to [Shivam Jha aka lzzy12](https://github.com/lzzy12) and [JaskaranSM aka Zero Cool](https://github.com/jaskaranSM) They build up this cheems from scratch.
- Then a huge thanks to [Sreeraj V R](https://github.com/SVR666) You can checkout his [repo here](https://github.com/SVR666/LoaderX-cheems)
...
This is a telegram cheems writen in python for mirroring files on the internet to our beloved Google Drive.

# Inspiration 
This project is heavily inspired from @out386 's telegram cheems which is written in JS.
...
## cheems commands to be set in cheemsfather
...
stats - cheems Usage Stats
help - Get Detailed Help
log - cheems Log [owner only]
```
...
- Clone this repo:
```
git clone https://github.com/magneto261290/magneto-python-aria mirror-cheems/
cd mirror-cheems
```
...
- **cheems_TOKEN** : The telegram cheems token that you get from @cheemsFather
- **GDRIVE_FOLDER_ID** : This is the folder ID of the Google Drive Folder to which you want to upload all the mirrors.
- **DOWNLOAD_DIR** : The path to the local folder where the downloads should be downloaded to
- **DOWNLOAD_STATUS_UPDATE_INTERVAL** : A short interval of time in seconds after which the Mirror progress message is updated. (I recommend to keep it 5 seconds at least)  
- **OWNER_ID** : The Telegram user ID (not username) of the owner of the cheems
- **AUTO_DELETE_MESSAGE_DURATION** : Interval of time (in seconds), after which the cheems deletes it's message (and command message) which is expected to be viewed instantly. Note: Set to -1 to never automatically delete messages
...
- **STOP_DUPLICATE_MIRROR**: (Optional field) (Leave empty if unsure) if this field is set to `True` , cheems will check file in drive, if it is present in drive, downloading will ne stopped. (Note - File will be checked using filename, not using filehash, so this feature is not perfect yet)
...
- Move that file to the root of mirror-cheems, and rename it to credentials.json
...
- Build Docker image:
```
sudo docker build . -t mirror-cheems
```
- Run the image:
```
sudo docker run mirror-cheems
```
